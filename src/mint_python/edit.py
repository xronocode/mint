# FILE: src/mint_python/edit.py
# VERSION: 1.0.0
# START_MODULE_CONTRACT
#   PURPOSE: Pure-Python port of `src/mint/edit.py` — DOCX edit pipeline that
#     accepts a typed EditPlan (JSON list of typed ops) and applies it via
#     deterministic XML transforms over an MP-OOXML unpack tree. Preserves the
#     legacy M-EDIT public surface byte-identically (EditPlan / EditOp /
#     TextAnchor / Anchor / EditMetadata / OpOutcome / EditResult dataclasses;
#     edit / validate_plan / resolve_anchor / extract_text_with_anchors /
#     build_edit_prompt / make_edit_metadata / edit_plan_from_dict /
#     render_diff public symbols; EditError exception with `code` attribute)
#     so the forthcoming MP-MCP-EDIT wrap (Phase-16 W3c) can swap import paths
#     cleanly and stay constraint-8 compliant.
#   SCOPE: validate plan -> backup -> unpack (MP-OOXML) -> extract anchors ->
#     resolve -> execute typed handlers in order -> pack -> run MP-VALIDATE ->
#     return EditResult with per-op diff. PPTX edits are EDIT_OP_UNSUPPORTED.
#     LLM call itself is OUT OF SCOPE for edit(); callers construct an
#     EditPlan then pass it in.
#   DEPENDS: stdlib (contextlib, datetime, hashlib, json, logging, shutil,
#     tempfile, time, unicodedata, uuid) + lxml >= 5.x + mint_python.ooxml
#     (W3a port) + mint_python.validate (W1 port; SeverityMode self-defined
#     there). NO `from mint.*` imports (constraint-8).
#   LINKS: docs/development-plan.xml#MP-EDIT,
#     docs/verification-plan.xml#V-MP-EDIT,
#     docs/knowledge-graph.xml#MP-EDIT
# END_MODULE_CONTRACT
#
# START_MODULE_MAP
#   EditError                    - exception with `code` attribute
#   TextAnchor                   - frozen dataclass; output of extract_text_with_anchors
#   Anchor                       - frozen dataclass; tagged on `type`
...
#   make_edit_metadata           - public: build EditMetadata for an EditPlan
#   edit_plan_from_dict          - public: construct EditPlan from JSON dict
...
#   _handle_replace_text         - op handler
#   _handle_insert_paragraph     - op handler
#   _handle_delete_paragraph     - op handler
#   _handle_set_paragraph_style  - op handler
#   _handle_tracked_replace      - op handler
#   _handle_tracked_delete       - op handler
#   _handle_add_comment          - op handler
#   _handle_accept_change        - op handler
#   _handle_reject_change        - op handler
#   _CommentsState               - in-memory aggregate for comments parts
# END_MODULE_MAP
#
# START_CHANGE_SUMMARY
#   LAST_CHANGE: Phase-16 Wave-16-3b initial port — mirrors mint.edit behavior
#     byte-for-byte on the same inputs (porting-parity scenario-8 is the
#     oracle). Log marker prefix changed from "Edit" to "MP-Edit" per
#     pure-python convention; BLOCK markers (BLOCK_EDIT_PLAN_VALIDATE,
#     BLOCK_EDIT_BACKUP, BLOCK_EDIT_EXTRACT_TEXT, BLOCK_EDIT_RESOLVE_ANCHOR,
#     BLOCK_EDIT_APPLY_OP, BLOCK_EDIT_TRACKED_CHANGE) preserved verbatim so
#     the DF-007 evidence chain stays intact. Imports re-pointed at
#     mint_python.ooxml (W3a) and mint_python.validate (W1; SeverityMode
#     self-defined there). Constraint-8: zero `from mint.*` lines.
# END_CHANGE_SUMMARY
...
from mint_python import ooxml as m_ooxml
from mint_python import validate as m_validate
...
# ---------------------------------------------------------------------------
# Namespaces (mirrors MP-OOXML; kept local so this module does not reach into
...
def _paragraph_hash(p: etree._Element) -> str:
    ...
    Tests monkeypatch this for the hash-collision scenario. Keep it as a
    simple module-level function so monkeypatch works without per-call
    indirection.
    """
...
#   LINKS: V-MP-EDIT scenario-3, scenario-4, scenario-9
# END_CONTRACT: validate_plan
def validate_plan(plan: EditPlan) -> None:
    # START_BLOCK_EDIT_PLAN_VALIDATE
    logger.info(
        f"[{_LOG_PREFIX}][validate_plan][BLOCK_EDIT_PLAN_VALIDATE] "
        f"start ops=%d format=%s",
        len(plan.ops),
        plan.format,
    )
    ...
    logger.info(
        f"[{_LOG_PREFIX}][validate_plan][BLOCK_EDIT_PLAN_VALIDATE] done ops=%d",
        len(plan.ops),
    )
...
def extract_text_with_anchors(unpack_dir: Path) -> list[TextAnchor]:
    ...
    logger.info(
        f"[{_LOG_PREFIX}][extract_text][BLOCK_EDIT_EXTRACT_TEXT] "
        f"start unpack_dir=%s",
        unpack_dir,
    )
    ...
    for part_name, candidates in _PART_PATHS.items():
        for rel in candidates:
            ...
            for p in tree.iter(f"{W}p"):
                ...
                counter += 1
    logger.info(
        f"[{_LOG_PREFIX}][extract_text][BLOCK_EDIT_EXTRACT_TEXT] done anchors=%d",
        len(anchors),
    )
    logger.debug(
        f"[{_LOG_PREFIX}][extract_text] full_anchor_list=%s",
...
#            that exact format. NEVER includes raw OOXML XML.
...
def build_edit_prompt(user_instruction: str, anchors: list[TextAnchor]) -> str:
    ...
    rendered = "\n".join(lines)
    logger.debug(
        f"[{_LOG_PREFIX}][build_edit_prompt] length=%d anchors=%d",
        len(rendered),
        len(anchors),
    )
    return rendered
...
def _resolve_anchor_with_state(
    ...
    for part_name, ta_list in candidates_by_part.items():
        ...
        logger.info(
            f"[{_LOG_PREFIX}][resolve_anchor][BLOCK_EDIT_RESOLVE_ANCHOR] "
            f"part=%s paragraph_index=%d hash=%s anchor_type=%s",
            part_name,
            ta.paragraph_index,
            ta.hash,
            anchor.type,
...
def _find_live_paragraph_by_identity(
    ...
    its content (and therefore its hash) is unchanged.
    """
...
#            -> pack -> run MP-VALIDATE -> return EditResult.
...
#   LINKS: V-MP-EDIT scenario-1, scenario-2, scenario-5, scenario-11
# END_CONTRACT: edit
def edit(
    ...
    # 3. backup BEFORE any tree mutation.
    # START_BLOCK_EDIT_BACKUP
    logger.info(
        f"[{_LOG_PREFIX}][backup][BLOCK_EDIT_BACKUP] start src=%s dst=%s",
        document_path.name,
        backup_path.name,
    )
    try:
    ...
    logger.info(
        f"[{_LOG_PREFIX}][backup][BLOCK_EDIT_BACKUP] done backup=%s bytes=%d",
        backup_path.name,
    ...
    with tempfile.TemporaryDirectory(prefix="mint_edit_") as tmp:
        ...
        # 4. unpack via MP-OOXML — emits BLOCK_OOXML_UNPACK.
        ...
        # 9. pack — emits BLOCK_OOXML_PACK.
        m_ooxml.pack(unpack_dir, output_path)

    # 10. validate the output via MP-VALIDATE — emits BLOCK_RUN_CHECKS.
    ...
    if effective_mode == SeverityMode.STRICT and not validation_report.passed:
        ...
        duration_ms = int((time.monotonic() - started) * 1000)
        raise EditError(
            f"Output failed MP-VALIDATE in strict mode: "
...
def _execute_op(
    ...
    logger.info(
        f"[{_LOG_PREFIX}][apply_op][BLOCK_EDIT_APPLY_OP] op_id=%s op_type=%s "
        f"part=%s outcome=success",
        op.op_id,
        op.type,
        part_name,
    )
    logger.debug(
        f"[{_LOG_PREFIX}][apply_op] op_id=%s before_len=%d after_len=%d",
        op.op_id,
        len(before_snippet),
        len(after_snippet),
    )
    ...
    if op.type in TRACKED_OP_TYPES:
        logger.info(
            f"[{_LOG_PREFIX}][tracked_change][BLOCK_EDIT_TRACKED_CHANGE] "
            f"op_id=%s op_type=%s target_part=%s",
            op.op_id,
            op.type,
            part_name,
        )
...
def make_edit_metadata(
    ...
    *,
    ...
    """Build EditMetadata with MP-EDIT-computed correlation_id and source_prompt_hash."""
    return EditMetadata(
