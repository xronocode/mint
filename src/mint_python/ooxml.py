# FILE: src/mint_python/ooxml.py
# VERSION: 1.0.0
# START_MODULE_CONTRACT
#   PURPOSE: Pure-Python port of mint.ooxml — pack/unpack of OOXML containers
#     (DOCX/PPTX/XLSX) with pretty-print, run merge, smart-quote escape, and
#     auto-repair. Preserves the legacy public API surface byte-identically
#     (OOXMLError, UnpackResult, PackResult, unpack, pack,
#     validate_relationships, merge_runs, escape_smart_quotes) so the
#     forthcoming MP-EDIT port (Phase-16 W3b) can swap import paths cleanly
#     and stay constraint-8 compliant.
#   SCOPE: Round-trip tree-equal XML parts and byte-equal binary parts;
#     preserve [Content_Types].xml Override entry order; cross-check
#     relationships; fail-fast on malformed input. No LibreOffice / soffice /
#     pandoc / shell-out — stdlib zipfile + lxml only.
#   DEPENDS: stdlib only (logging, re, unicodedata, zipfile, dataclasses,
#     pathlib, typing) + lxml >= 5.x. NO mint.* imports (constraint-8).
#   LINKS: docs/development-plan.xml#MP-OOXML,
#     docs/verification-plan.xml#V-MP-OOXML,
#     docs/knowledge-graph.xml#MP-OOXML
# END_MODULE_CONTRACT
#
# START_MODULE_MAP
#   OOXMLError                       - base exception with `code` attribute
#   UnpackResult                     - frozen dataclass returned by unpack()
#   PackResult                       - frozen dataclass returned by pack()
#   _PackStats                       - internal mutable counter container
#   unpack                           - public: extract DOCX/PPTX zip into dir
#   pack                             - public: re-zip unpack_dir into output
#   merge_runs                       - public: merge adjacent w:r with equal rPr
#   escape_smart_quotes              - public: alternate quotes inside w:t
#   validate_relationships           - public: cross-check rels Targets resolve
#   _validate_relationships          - internal: actual rels walk
#   _rels_owner_dir                  - internal: derive owner dir for rels path
#   _resolve_target                  - internal: resolve a rel Target
#   _process_xml_for_pack            - internal: parse + repair + serialize
#   _merge_runs_in_tree              - internal: tree-level run merge
#   _merge_runs_in_paragraph         - internal: per-paragraph run merge
#   _are_runs_mergeable              - internal: boundary + rPr equality check
#   _merge_run_pair                  - internal: fold run b into run a
#   _elements_equal                  - internal: structural element equality
#   _escape_smart_quotes_in_tree     - internal: tree-level quote escape
#   _escape_smart_quotes_in_paragraph- internal: per-paragraph quote escape
#   _run_is_code_styled              - internal: detect Code-style run/para
#   _autorepair_durable_ids          - internal: regen w:durableId out of range
#   _autorepair_whitespace_preserve  - internal: add xml:space=preserve
#   _is_binary_part                  - internal: classify binary parts
#   _is_xml_part                     - internal: classify XML parts
#   _should_transform                - internal: gate for transforms
#   _format_from_parts               - internal: detect docx/pptx/xlsx
# END_MODULE_MAP
#
# START_CHANGE_SUMMARY
#   LAST_CHANGE: Phase-16 Wave-16-3a initial port — mirrors mint.ooxml
#     behavior byte-for-byte on the same inputs (porting-parity scenario-7
#     is the oracle). Log marker prefix changed from "OOXML" to "MP-OOXML"
#     per pure-python convention; BLOCK markers (BLOCK_OOXML_UNPACK /
#     BLOCK_OOXML_PACK / BLOCK_OOXML_AUTOREPAIR) preserved verbatim so the
#     DF-007 evidence chain stays intact. Determinism strategy for
#     _autorepair_durable_ids: monotonic counter seeded at 1 with collision
#     skipping (identical to legacy); same input → same output id sequence.
# END_CHANGE_SUMMARY
...
# Internal mutable counter container. Frozen dataclasses are returned to
# callers; this one is private.
@dataclass
class _PackStats:
    ...
    *,
    ...
    if not document_path.exists():
        raise OOXMLError(
            ...
            code="OOXML_NOT_A_ZIP",
        )

    # START_BLOCK_OOXML_UNPACK
    logger.info(
        f"[{_LOG_PREFIX}][unpack][BLOCK_OOXML_UNPACK] start path=%s "
        "merge_runs=%s pretty_print=%s",
        document_path,
        merge_runs,
        pretty_print,
    )
    ...
    if not zipfile.is_zipfile(document_path):
        raise OOXMLError(
            f"Not a zip file: {document_path}",
            code="OOXML_NOT_A_ZIP",
        )
    ...
    except zipfile.BadZipFile as exc:  # pragma: no cover - TOCTOU defensive (is_zipfile passed)
        raise OOXMLError(
            f"Not a valid zip file: {exc}",
            code="OOXML_NOT_A_ZIP",
        ) from exc
    ...
    logger.info(
        f"[{_LOG_PREFIX}][unpack][BLOCK_OOXML_UNPACK] done parts=%d "
        "runs_merged=%d quotes_escaped=%d format=%s",
        len(parts),
        runs_merged_total,
...
def pack(unpack_dir: Path, output_path: Path) -> PackResult:
    ...
    if not unpack_dir.exists():
        raise OOXMLError(
            f"Unpack dir not found: {unpack_dir}",
            code="OOXML_PACK_FAILED",
        )

    # START_BLOCK_OOXML_PACK
    logger.info(
        f"[{_LOG_PREFIX}][pack][BLOCK_OOXML_PACK] start unpack_dir=%s output=%s",
        unpack_dir,
        output_path,
    )
    ...
    # [Content_Types].xml Override entries — this is what V-MP-OOXML
    # scenario-2 enforces. The walk of unpack_dir is otherwise arbitrary on
    ...
    if not all_files:
        raise OOXMLError(
            f"Unpack dir is empty: {unpack_dir}",
            code="OOXML_PACK_FAILED",
        )
    ...
    logger.info(
        f"[{_LOG_PREFIX}][pack][BLOCK_OOXML_PACK] done output=%s bytes=%d "
...
def _process_xml_for_pack(
    ...
    if rel == "[Content_Types].xml":
        ...
        stats.overrides_order = [
            el.get("PartName", "") for el in tree.findall(f"{CT}Override")
        ]
        ...
        if (
            ...
            logger.info(
                f"[{_LOG_PREFIX}][autorepair][BLOCK_OOXML_AUTOREPAIR] "
                "part=%s repaired_durable_ids+=%d preserved_whitespace_runs+=%d",
                rel,
                stats.repaired_durable_ids - before_d,
...
def _rels_owner_dir(rels_posix: str) -> str:
    ...
    if rels_posix == "_rels/.rels":
        return ""
    return ""  # pragma: no cover - unreachable: all rels files match one of the two patterns above
...
def _escape_smart_quotes_in_paragraph(p: etree._Element) -> int:
    ...
    # First pass: determine whether the total number of double quotes is odd.
...
def _format_from_parts(parts: Iterable[str]) -> Literal["docx", "pptx", "xlsx"]:
    ...
    if any(p.startswith("ppt/") for p in parts_list):
        return "pptx"
    # xlsx branch is defensive — not exercised by current docx/pptx fixtures.
    if any(p.startswith("xl/") for p in parts_list):  # pragma: no cover
        return "xlsx"
    # Default to docx if undetectable; unreachable in practice because
    # [Content_Types].xml will identify the format.
    return "docx"  # pragma: no cover - unreachable


# Suppress unused-import warning under strict linters: unicodedata is kept
# for future quote-normalization passes and parity with the legacy module's
# import list.
_ = unicodedata


# Convenience alias retained for future test introspection / parity with the
# legacy module's public introspection surface.
