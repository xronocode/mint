...
def render_assembly_template(
    ...
    for spec in plan_data.sections:
        if spec.type == "cover":
            # Enrich the cover section to fill the empty whitespace below the
            # title — VLM consistently flags the cover as "title alone on a
            # mostly empty page". The model's code goes first, then we add a
            # decorative horizontal rule + a footer band so the page has
            # visual closure rather than ending mid-page in white.
            base_code = sections_code.get(spec.id, "")
            if not base_code or spec.id in sections_placeholder:
                base_code = make_placeholder(spec)
            base_code = base_code.rstrip().rstrip(",").rstrip()
            cover_decor = (
                "new Paragraph({ spacing: { before: 240 }, children: [] }),\n"
                "new Paragraph({ "
                "border: { bottom: { style: BorderStyle.SINGLE, size: 12, "
                "color: '2E75B6', space: 1 } }, "
                "children: [] }),\n"
                "new Paragraph({ spacing: { before: 7200 }, children: [] }),\n"
                "new Paragraph({ "
