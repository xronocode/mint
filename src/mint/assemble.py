...
def render_assembly_template(
    ...
    # Always-on running header + paginated footer. Chrome strings come
    # from plan.header_footer (M-PLAN derives them from doc title/
    # audience/purpose). Fallbacks only apply when the planner left a
    # field empty: header_left → cover.title; header_right → omitted.
    cover_spec = next(
        (s for s in plan_data.sections if s.type == "cover"),
        plan_data.sections[0] if plan_data.sections else None,
    )
    hf = plan_data.header_footer
    doc_title = (hf.header_left or "").strip()
    if not doc_title:
        doc_title = cover_spec.title if cover_spec else "Document"
    header_right_text = (hf.header_right or "").strip()
    ...
    if header_right_text:
        right_runs = (
            "    new TextRun({ text: '\\t' }),\n"
            f"    new TextRun({{ text: {json.dumps(header_right_text)}, "
    ...
    for spec in plan_data.sections:
        if spec.type == "cover":
            ...
            code = (
                ...
                "  children: [],\n"
                "}),\n"
                # Tagline (centered, italic, muted). When cover_metadata
                # is present, after-spacing of 2880 (~2 inches) pushes the
                # metadata to the bottom of the cover. With no metadata
                # paragraph below, we leave a smaller gap.
                + (
                    "new Paragraph({\n"
                    "  alignment: AlignmentType.CENTER,\n"
                    + (
                        "  spacing: { after: 2880 },\n"
                        if hf.cover_metadata.strip()
                        else "  spacing: { after: 360 },\n"
                    )
                    + "  children: [new TextRun({\n"
                    f"    text: '{tagline}',\n"
                ...
                + (
                    # Bottom metadata footer (only if the planner provided one).
                    ",\nnew Paragraph({\n"
                    "  alignment: AlignmentType.CENTER,\n"
                    "  children: [new TextRun({\n"
                    f"    text: {json.dumps(hf.cover_metadata.strip())},\n"
